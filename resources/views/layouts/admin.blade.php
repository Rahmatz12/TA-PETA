...
<head>
    ...
    <title>@yield('title', 'Admin Panel - SIG Irigasi Sidoarjo')</title>
        ...
        <aside class="sidebar-admin" id="adminSidebar">
            ...
            <nav class="sidebar-nav">
                <div class="nav-section">
                    ...
                    <a href="{{ route('admin.users.index') }}" class="nav-item {{ request()->routeIs('admin.users.*') ? 'active' : '' }}"><i class="fas fa-users-cog"></i> Kelola Pengguna</a>
                    <a href="{{ route('admin.laporan.index') }}" class="nav-item {{ request()->routeIs('admin.laporan.*') ? 'active' : '' }}"><i class="fas fa-clipboard-list"></i> Laporan Masyarakat</a>
                </div>
                <div class="nav-section">
                    <div class="nav-section-title">Data</div>
                    <a href="{{ route('admin.data-irigasi.index') }}" class="nav-item {{ request()->routeIs('admin.data-irigasi.*') ? 'active' : '' }}"><i class="fas fa-database"></i> Data Irigasi</a>
                    <a href="{{ route('admin.peta.index') }}" class="nav-item {{ request()->routeIs('admin.peta.*') ? 'active' : '' }}"><i class="fas fa-map"></i> Peta Interaktif</a>
                </div>
            </nav>
            <div class="sidebar-footer">
                <a href="{{ route('admin.profil') }}" class="admin-user" style="text-decoration:none;color:inherit">
                    <div class="admin-avatar">{{ substr(Auth::guard('admin')->user()->name, 0, 1) }}</div>
                    <div class="admin-user-info">
                        <h4>{{ Auth::guard('admin')->user()->name }}</h4>
                        <p>{{ ucfirst(Auth::guard('admin')->user()->role) }}</p>
                    </div>
                </a>
            </div>
        </aside>
        ...
        <div class="main-admin">
            <div class="topbar">
                ...
                <div class="topbar-right">
                    <a href="{{ route('home') }}" class="topbar-action" title="Lihat Website"><i class="fas fa-globe"></i></a>
                    <a href="{{ route('admin.profil') }}" class="topbar-action" title="Profil"><i class="fas fa-user"></i></a>
                    <a href="{{ route('admin.logout') }}" class="topbar-action" title="Keluar" onclick="event.preventDefault(); document.getElementById('admin-logout-form').submit();"><i class="fas fa-sign-out-alt"></i></a>
                </div>
            </div>
