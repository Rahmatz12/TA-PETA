...
@section('page-title', 'Data Irigasi')
...
<div class="stats-row">
    <div class="sr-card">
        ...
        <div class="sr-info"><h4>{{ $stats['total'] }}</h4><p>Total Saluran</p></div>
    </div>
    <div class="sr-card">
        <div class="sr-icon" style="background:#dcfce7;color:#16a34a"><i class="fas fa-check-circle"></i></div>
        <div class="sr-info"><h4>{{ $stats['baik'] }}</h4><p>Saluran Baik</p></div>
    </div>
    <div class="sr-card">
        <div class="sr-icon" style="background:#fee2e2;color:#b91c1c"><i class="fas fa-exclamation-triangle"></i></div>
        <div class="sr-info"><h4>{{ $stats['rusak_berat'] + $stats['rusak_sedang'] + $stats['rusak_ringan'] }}</h4><p>Bermasalah</p></div>
    </div>
</div>

<form method="GET" action="{{ route('admin.data-irigasi.index') }}" class="filter-bar">
    ...
    <input type="text" name="search" class="filter-input" placeholder="Cari nama atau kecamatan..." value="{{ request('search') }}">
    <button type="submit" class="btn-filter"><i class="fas fa-search"></i> Cari</button>
    <a href="{{ route('admin.data-irigasi.index') }}" class="btn-filter" style="background:#94a3b8;text-decoration:none"><i class="fas fa-sync-alt"></i> Reset</a>
...
<div class="card-white">
        ...
        <table class="data-table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td><span class="j-badge j-{{ $s->jenis }}">{{ ucfirst($s->jenis) }}</span></td>
                    <td>{{ $s->kecamatan }}</td>
                    <td>
                        @php $sc = match($s->kondisi){'baik'=>'s-baik','perbaikan'=>'s-perbaikan','rusak-berat'=>'s-rusak-berat','rusak-sedang'=>'s-rusak-sedang','rusak-ringan'=>'s-rusak-ringan',default=>'s-baik'}; @endphp
                        <span class="s-badge {{ $sc }}"><span style="width:6px;height:6px;border-radius:50%;background:currentColor"></span> {{ ucfirst(str_replace('-', ' ', $s->kondisi)) }}</span>
                    </td>
                    <td><span style="color:{{ $s->status=='aktif'?'#16a34a':'#ef4444' }};font-weight:600;font-size:0.8rem"><i class="fas fa-circle" style="font-size:0.5rem"></i> {{ ucfirst($s->status) }}</span></td>
                    <td>{{ $s->level_air ?? '-' }}</td>
                    <td>
                        <div style="display:flex;gap:0.3rem">
                            <button class="btn-sm btn-edit" onclick="openEditModal({{ $s->id }})"><i class="fas fa-edit"></i> Edit</button>
                            <form method="POST" action="{{ route('admin.data-irigasi.destroy', $s->id) }}" style="display:inline" onsubmit="return confirm('Yakin ingin menghapus?')">
                                @csrf @method('DELETE')
    ...
    <div style="padding:1rem 1.5rem;border-top:1px solid #f1f5f9">{{ $salurans->links() }}</div>
</div>
...
<div class="modal-overlay" id="modalTambah">
    <div class="modal-box">
        ...
        <form method="POST" action="{{ route('admin.data-irigasi.store') }}">
            @csrf
            <div class="form-row">
...
<script>
    const saluranData = @json($salurans->items());
    ...
    function openEditModal(id) {
        ...
        if (!s) return;
        document.getElementById('formEdit').action = '{{ url('admin/data-irigasi') }}/' + id;
