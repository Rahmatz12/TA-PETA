...
<div class="map-page">
    ...
    <div class="map-sidebar">
        ...
        <div class="ms-stats">
            <div class="ms-stat"><h4>{{ $total }}</h4><p>Total</p></div>
            <div class="ms-stat"><h4>{{ $aktif }}</h4><p>Aktif</p></div>
            <div class="ms-stat"><h4>{{ $total - $aktif }}</h4><p>Nonaktif</p></div>
        </div>
        <div class="ms-list" id="msList">
            @foreach($salurans as $s)
            <div class="ms-item" id="ms-{{ $s->id }}" onclick="focusSaluran({{ $s->id }})">
                <div class="ms-icon" style="background:{{ match($s->jenis){'primer'=>'#e0f2fe','sekunder'=>'#dcfce7','tersier'=>'#fef3c7','pintu'=>'#ede9fe','bendungan'=>'#f3e8ff',default=>'#e0f2fe'} }};color:{{ match($s->jenis){'primer'=>'#0369a1','sekunder'=>'#16a34a','tersier'=>'#d97706','pintu'=>'#7c3aed','bendungan'=>'#9333ea',default=>'#0369a1'} }}">
                ...
                <div class="ms-info">
                    <h4>{{ $s->nama }}</h4>
                    <p>{{ $s->kecamatan }} - {{ ucfirst($s->jenis) }}</p>
                </div>
                <span class="ms-badge" style="background:{{ $s->kondisi=='baik'?'#dcfce7':($s->kondisi=='perbaikan'?'#f3e8ff':'#fee2e2') }};color:{{ $s->kondisi=='baik'?'#16a34a':($s->kondisi=='perbaikan'?'#7c3aed':'#b91c1c') }}">{{ ucfirst(str_replace('-',' ',$s->kondisi)) }}</span>
            </div>
            @endforeach
        </div>
    </div>
