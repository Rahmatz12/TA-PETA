...
@section('title', 'Dashboard Admin - SIG Irigasi Sidoarjo')
...
<div class="dashboard-stats">
    <div class="dstat-card">
        ...
        <div class="dstat-info"><h3>{{ $stats['total_saluran'] }}</h3><p>Total Saluran</p></div>
    </div>
    <div class="dstat-card">
        <div class="dstat-icon" style="background:#fef3c7;color:#d97706"><i class="fas fa-clipboard-list"></i></div>
        <div class="dstat-info"><h3>{{ $stats['total_laporan'] }}</h3><p>Total Laporan</p></div>
    </div>
    <div class="dstat-card">
        <div class="dstat-icon" style="background:#dcfce7;color:#16a34a"><i class="fas fa-users"></i></div>
        <div class="dstat-info"><h3>{{ $stats['total_masyarakat'] }}</h3><p>Masyarakat</p></div>
    </div>
    <div class="dstat-card">
        <div class="dstat-icon" style="background:#ede9fe;color:#7c3aed"><i class="fas fa-user-shield"></i></div>
        <div class="dstat-info"><h3>{{ $stats['total_admin'] }}</h3><p>Admin/Petugas</p></div>
    </div>
</div>
    ...
    <div>
        <div class="dash-card" style="margin-bottom:1.5rem">
            ...
            <div class="dash-card-body">
                @php $totalKondisi = array_sum($kondisiStats); @endphp
                <div class="kondisi-bar">
                    <div class="label">Baik</div>
                    <div class="bar-wrap"><div class="bar-fill" style="width:{{ $totalKondisi > 0 ? ($kondisiStats['baik']/$totalKondisi)*100 : 0 }}%;background:#16a34a"></div></div>
                    <div class="count">{{ $kondisiStats['baik'] }}</div>
                </div>
                <div class="kondisi-bar">
                    <div class="label">Perbaikan</div>
                    <div class="bar-wrap"><div class="bar-fill" style="width:{{ $totalKondisi > 0 ? ($kondisiStats['perbaikan']/$totalKondisi)*100 : 0 }}%;background:#a855f7"></div></div>
                    <div class="count">{{ $kondisiStats['perbaikan'] }}</div>
                </div>
                <div class="kondisi-bar">
                    <div class="label">Rusak Ringan</div>
                    <div class="bar-wrap"><div class="bar-fill" style="width:{{ $totalKondisi > 0 ? ($kondisiStats['rusak_ringan']/$totalKondisi)*100 : 0 }}%;background:#3b82f6"></div></div>
                    <div class="count">{{ $kondisiStats['rusak_ringan'] }}</div>
                </div>
                <div class="kondisi-bar">
                    <div class="label">Rusak Sedang</div>
                    <div class="bar-wrap"><div class="bar-fill" style="width:{{ $totalKondisi > 0 ? ($kondisiStats['rusak_sedang']/$totalKondisi)*100 : 0 }}%;background:#eab308"></div></div>
                    <div class="count">{{ $kondisiStats['rusak_sedang'] }}</div>
                </div>
                <div class="kondisi-bar">
                    <div class="label">Rusak Berat</div>
                    <div class="bar-wrap"><div class="bar-fill" style="width:{{ $totalKondisi > 0 ? ($kondisiStats['rusak_ringan']/$totalKondisi)*100 : 0 }}%;background:#ef4444"></div></div>
                    <div class="count">{{ $kondisiStats['rusak_berat'] }}</div>
                </div>
            </div>
        </div>
            ...
            <div class="dash-card-body">
                ...
                <div class="list-item-sm">
                    ...
                    <div class="list-info-sm" style="flex:1">
                        <h4>{{ Str::limit($laporan->jenis_kerusakan, 40) }}</h4>
                        <p>{{ $laporan->nama_saluran }} - {{ $laporan->created_at->format('d M Y') }}</p>
                    </div>
                    @php
                    ...
                    @endphp
                    <span class="laporan-badge {{ $lbClass }}">{{ ucfirst($laporan->status) }}</span>
                </div>
                @empty
    ...
    <div>
            ...
            <div class="dash-card-body">
                <div class="jenis-pie">
                    ...
                    <div class="pie-legend">
                        ...
                        <div class="pie-item">
                            <div class="pie-dot" style="background:{{ ['#163832','#235347','#8EB69B','#0B2B26','#DAF1DE'][array_search($jenis, array_keys($jenisStats))] }}"></div>
                            <span>{{ ucfirst($jenis) }} ({{ $count }})</span>
                        </div>
                        @endforeach
                    </div>
                </div>
        ...
        <div class="dash-card">
            ...
            <div class="dash-card-body">
                ...
                <div class="list-item-sm">
                    <div class="list-avatar-sm" style="background:#e0f2fe;color:#0369a1;font-weight:700">{{ substr($user->name, 0, 1) }}</div>
...
<style>
    #jenisPie { background: conic-gradient(
        #163832 0deg {{ $jenisStats['primer'] > 0 ? ($jenisStats['primer']/array_sum($jenisStats))*360 : 0 }}deg,
        #235347 {{ $jenisStats['primer'] > 0 ? ($jenisStats['primer']/array_sum($jenisStats))*360 : 0 }}deg {{ $jenisStats['primer']+$jenisStats['sekunder'] > 0 ? (($jenisStats['primer']+$jenisStats['sekunder'])/array_sum($jenisStats))*360 : 0 }}deg,
        #8EB69B {{ $jenisStats['primer']+$jenisStats['sekunder'] > 0 ? (($jenisStats['primer']+$jenisStats['sekunder'])/array_sum($jenisStats))*360 : 0 }}deg {{ $jenisStats['primer']+$jenisStats['sekunder']+$jenisStats['tersier'] > 0 ? (($jenisStats['primer']+$jenisStats['sekunder']+$jenisStats['tersier'])/array_sum($jenisStats))*360 : 0 }}deg,
        #0B2B26 {{ $jenisStats['primer']+$jenisStats['sekunder']+$jenisStats['tersier'] > 0 ? (($jenisStats['primer']+$jenisStats['sekunder']+$jenisStats['tersier'])/array_sum($jenisStats))*360 : 0 }}deg {{ array_sum($jenisStats)-$jenisStats['bendungan'] > 0 ? ((array_sum($jenisStats)-$jenisStats['bendungan'])/array_sum($jenisStats))*360 : 0 }}deg,
        #DAF1DE {{ array_sum($jenisStats)-$jenisStats['bendungan'] > 0 ? ((array_sum($jenisStats)-$jenisStats['bendungan'])/array_sum($jenisStats))*360 : 0 }}deg 360deg
    ); }
</style>
@endsection
