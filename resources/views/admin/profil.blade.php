...
@section('title', 'Profil Admin - SIG Irigasi Sidoarjo')
...
<div class="profile-card">
    ...
    <div class="profile-body">
        <div class="profile-avatar">{{ substr($admin->name, 0, 1) }}</div>
        <div class="profile-name">
            <h2>{{ $admin->name }}</h2>
            <p>{{ $admin->email }}</p>
            <span class="role-badge"><i class="fas fa-{{ $admin->role=='admin'?'shield-alt':'user-cog' }}"></i> {{ ucfirst($admin->role) }}</span>
        </div>
        ...
        <div id="tab-profil" class="tab-content active">
            <form method="POST" action="{{ route('admin.profil.update') }}">
                @csrf @method('PUT')
                <div class="info-row">
                    <div class="form-group"><label>Nama Lengkap</label><input type="text" name="name" class="form-control" value="{{ $admin->name }}" required></div>
                    <div class="form-group"><label>Email</label><input type="email" class="form-control" value="{{ $admin->email }}" readonly></div>
                </div>
                <div class="info-row">
                    <div class="form-group"><label>No. Telepon</label><input type="tel" name="phone" class="form-control" value="{{ $admin->phone }}"></div>
                    <div class="form-group"><label>Peran</label><input type="text" class="form-control" value="{{ ucfirst($admin->role) }}" readonly></div>
                </div>
                <div style="display:flex;gap:1rem;">
                    <button type="submit" class="btn-save"><i class="fas fa-save"></i> Simpan Perubahan</button>
                    <a href="{{ route('admin.logout') }}" class="btn-logout" onclick="event.preventDefault(); document.getElementById('admin-logout-form').submit();"><i class="fas fa-sign-out-alt"></i> Keluar</a>
                </div>
            </form>
        </div>
