<?php $__env->startSection('title', 'Laporan Masyarakat - SIG Irigasi Sidoarjo'); ?>
<?php $__env->startSection('page-title', 'Laporan Masyarakat'); ?>
...
<div class="stats-row">
    <div class="sr-card"><h4 style="color:#0369a1"><?php echo e($stats['total']); ?></h4><p>Total Laporan</p></div>
    <div class="sr-card"><h4 style="color:#d97706"><?php echo e($stats['menunggu']); ?></h4><p>Menunggu</p></div>
    <div class="sr-card"><h4 style="color:#16a34a"><?php echo e($stats['diverifikasi']); ?></h4><p>Diverifikasi</p></div>
    <div class="sr-card"><h4 style="color:#7c3aed"><?php echo e($stats['diproses']); ?></h4><p>Diproses</p></div>
    <div class="sr-card"><h4 style="color:#1d4ed8"><?php echo e($stats['selesai']); ?></h4><p>Selesai</p></div>
    <div class="sr-card"><h4 style="color:#b91c1c"><?php echo e($stats['ditolak']); ?></h4><p>Ditolak</p></div>
</div>

<form method="GET" action="<?php echo e(route('admin.laporan.index')); ?>" class="filter-bar">
    ...
    <input type="text" name="search" class="filter-input" placeholder="Cari kode, nama pelapor, atau saluran..." value="<?php echo e(request('search')); ?>">
    <button type="submit" class="btn-filter"><i class="fas fa-search"></i> Cari</button>
    <a href="<?php echo e(route('admin.laporan.index')); ?>" class="btn-filter" style="background:#94a3b8;text-decoration:none"><i class="fas fa-sync-alt"></i> Reset</a>
</form>
...
<div class="card-white">
    ...
    <div style="overflow-x:auto">
        <table class="data-table">
            ...
            <tbody>
                ...
                <tr>
                    <td><strong style="font-family:monospace;color:var(--primary)"><?php echo e($laporan->kode_laporan); ?></strong></td>
                    <td><?php echo e($laporan->is_anonymous ? 'Anonim' : $laporan->nama_pelapor); ?></td>
                    <td><?php echo e($laporan->nama_saluran); ?></td>
                    <td><?php echo e($laporan->kecamatan); ?></td>
                    <td>
                        <?php $pc = match($laporan->tingkat_keparahan){'ringan'=>'p-ringan','sedang'=>'p-sedang','berat'=>'p-berat','kritis'=>'p-kritis',default=>'p-ringan'}; ?>
                        <span class="p-badge <?php echo e($pc); ?>"><?php echo e(ucfirst($laporan->tingkat_keparahan)); ?></span>
                    </td>
                    <td>
                        <?php $sc = match($laporan->status){'menunggu'=>'s-menunggu','diverifikasi'=>'s-diverifikasi','diproses'=>'s-diproses','selesai'=>'s-selesai','ditolak'=>'s-ditolak',default=>'s-menunggu'}; ?>
                        <span class="badge-status <?php echo e($sc); ?>"><span style="width:6px;height:6px;border-radius:50%;background:currentColor"></span> <?php echo e(ucfirst($laporan->status)); ?></span>
                    </td>
                    <td><?php echo e($laporan->created_at->format('d M Y')); ?></td>
                    <td>
                        <div class="action-btns">
                            <?php if($laporan->status == 'menunggu'): ?>
                                <button class="btn-action btn-verify" onclick="openActionModal('verifikasi', <?php echo e($laporan->id); ?>)"><i class="fas fa-check"></i> Verifikasi</button>
                                <button class="btn-action btn-reject" onclick="openActionModal('tolak', <?php echo e($laporan->id); ?>)"><i class="fas fa-times"></i> Tolak</button>
                            <?php elseif($laporan->status == 'diverifikasi'): ?>
                                <button class="btn-action btn-process" onclick="openActionModal('proses', <?php echo e($laporan->id); ?>)"><i class="fas fa-tools"></i> Proses</button>
                            <?php elseif($laporan->status == 'diproses'): ?>
                                <button class="btn-action btn-done" onclick="openActionModal('selesai', <?php echo e($laporan->id); ?>)"><i class="fas fa-check-double"></i> Selesai</button>
                            <?php endif; ?>
                        </div>
                    </td>
                </tr>
    ...
    <div style="padding:1rem 1.5rem;border-top:1px solid #f1f5f9"><?php echo e($laporans->links()); ?></div>
</div>
...
<script>
    function openActionModal(type, id) {
        closeModals();
        const base = '<?php echo e(url('admin/laporan')); ?>';
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\Rahmat Eka\Peta-Interaktif\resources\views/admin/laporan.blade.php ENDPATH**/ ?>